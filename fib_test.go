...
// Other _test files have godoc examples and import fib_test to use the
// exported API.  This _test file imports fib to see the unexported structure.
// The file contains:
// * Heap validation functions
// * Heap construction functions
// * Test cases from F&T 1987
// * Addition tests for coverage
...
// helper for Heap.validate.  depth first recursive.
...
// handy for use inside of Heap.str
...
// Heap.str formats a human readable representation of a Heap.
...
// helper fun constructing arbitrary Heaps.
...
// An int Value for validating examples from F&T 1987.
type Int int
...
func TestMeldToEmpty(t *testing.T) {
	h := &Heap{}
	h2 := &Heap{}
	h2.Insert(Int(3))
	h.Meld(h2)
...
roots:
3:
  parent, child, prev, next: <nil> <nil> 3 3
  rank, mark: 0 false
level 1:
`
	if got != want {
...
2:
  ...
  rank, mark: 0 false
3:
  parent, child, prev, next: <nil> 4 2 2
  rank, mark: 1 false
level 1:
4:
  parent, child, prev, next: 3 <nil> 4 4
  rank, mark: 0 false
level 2:
`
	if got != want {
...
func TestDecreaseKey(t *testing.T) {
	h := &Heap{}
	n := h.Insert(Int(3))
	if h.DecreaseKey(n, Int(4)) == nil {
		t.Fatal("DecreaseKey with larger key returned nil, want non-nil error")
	}
	if err := h.DecreaseKey(n, Int(2)); err != nil {
		t.Fatal(err)
	}
	got := h.str()
	want := `min value (top of heap) 2
roots:
2:
  parent, child, prev, next: <nil> <nil> 2 2
  rank, mark: 0 false
level 1:
`
	if got != want {
